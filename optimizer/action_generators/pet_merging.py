...
def generate_actions(player_state: Any, knowledge: dict[str, Any], options: dict[str, Any] | None = None):
    return generate_known_record_actions(
        player_state=player_state, records=knowledge.get("pet_merging", []), system="pet_merging",
        action_type="merge_pet", affected_field="affected_pet", include_saves=(options or {}).get("include_saves", True),
    )
